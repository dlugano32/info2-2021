...
ShSem &ShSem::operator <<(const int cant)
{
    escribir((void *) &cant, sizeof(int),indc);     //Se supone que el numero deberia ir al principio de las structs

    return *this;
}


ShSem & ShSem::operator <<(const struct sensor &msg)
{
    escribir((void *) &msg, sizeof(sensor), indc);
...
ShSem &ShSem::operator >>(const int *cant)
{
    leer((void *) cant, sizeof(int), indc);
...
ShSem & ShSem::operator >>(const struct sensor &msg)
{
    leer( (void *) &msg, sizeof (sensor), indc);
