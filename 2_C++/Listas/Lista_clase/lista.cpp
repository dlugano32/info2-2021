...
//Agregar nodo principio y agregar nodo final podrian llamar a esta funcion pero las dejo como ejemplo

Lista &Lista::AgregarElemento(const Persona &P, int pos)
{
    nodo * nuevo =new nodo;
    ...
    else
    {
        ...
        for(int i=0; i<pos-1; i++)
            aux=aux->next;      //Recorro la lista hasta llegar a la posicion que quiero

        if(pos-1==tam)    //Osea estaria en el ultimo
            nuevo->next=nullptr;
        else
            nuevo->next=aux->next;
