        ...
        *(arrows + 3) = &u;
        ...
        do
        {
                while (al_get_next_event(queue, &inEvent))
                {
                        ...
                        case ALLEGRO_EVENT_KEY_DOWN:
                                if (inEvent.keyboard.display == game)
                                {
                                        ...
                                        {
                                                score += r.pop(inEvent.keyboard.timestamp - tini);
                                                held[0] = true;
                                        }
                                        if (al_key_down(&keyboard, ALLEGRO_KEY_DOWN) && !held[1])
                                        {
                                                score += d.pop(inEvent.keyboard.timestamp - tini);
                                                held[1] = true;
                                        }
                                        if (al_key_down(&keyboard, ALLEGRO_KEY_LEFT) && !held[2])
                                        {
                                                score += l.pop(inEvent.keyboard.timestamp - tini);
                                                held[2] = true;
                                        }
                                        if (al_key_down(&keyboard, ALLEGRO_KEY_UP) && !held[3])
                                        {
                                                score += u.pop(inEvent.keyboard.timestamp - tini);
                                                held[3] = true;
                                        }
                        ...
                        case ALLEGRO_EVENT_DANCEPAD_DOWN:
                                ...
                                case ALLEGRO_KEY_RIGHT:
                                        score += r.pop(inEvent.touch.x);
                                        printf("%lf\n", inEvent.touch.x);
                                        break;
                                case ALLEGRO_KEY_DOWN:
                                        score += d.pop(inEvent.touch.x);
                                        printf("%lf\n", inEvent.touch.x);
                                        break;
                                case ALLEGRO_KEY_LEFT:
                                        score += l.pop(inEvent.touch.x);
                                        printf("%lf\n", inEvent.touch.x);
                                        break;
                                case ALLEGRO_KEY_UP:
                                        score += u.pop(inEvent.touch.x);
                                        printf("%lf\n", inEvent.touch.x);
                                        break;
                                }
                                break;
                ...
                if (fr ? (tn - tlast >= 1 / fr) : 1)
                {
                        ...
                        (!r.isEmpty()) ? (r.update(score) ? r.draw() : nop()) : nop();
                        (!d.isEmpty()) ? (d.update(score) ? d.draw() : nop()) : nop();
                        (!l.isEmpty()) ? (l.update(score) ? l.draw() : nop()) : nop();
                        (!u.isEmpty()) ? (u.update(score) ? u.draw() : nop()) : nop();
                        
                        if(score < 0) 
                                score = 0;

                        if (score + (cant - (r.popped + d.popped + l.popped + u.popped)) * 100 < cant * 70)
                        {
                                loop = false;
                                score = 0;
                        }
